...
public class AddDrugsActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((TextView)v.findViewById(R.id.myTitleTxt)).setText("Drug - "+title);
    ...
    private long addDrug(int drugID) {
        ...
        cv.put(db.DRUG_SOW_PERSOW_OPTION, "0000000000000000000000");
        cv.put(db.DRUG_SOW_PERSOW_COST, 0.00);
        cv.put(db.DRUG_SOW_PERSOW_TOTAL, 0.00);


        long id;
        if(title.equals("Per Sow")) {
            id = db.insertRow(db.DRUG_SOW_PERSOW_TABLE, cv);
        }else{
            id = db.insertRow(db.DRUG_PIG_TABLE, cv);
        }
        if(id>0){
            return 1;
        }else{
    ...
    public List<medicineChapter> getDataForListView()
    {
        ...
        db.open();
        Cursor cursor;
        cursor = db.getAll(db.DRUG_TABLE, "asc");
        cursor.requery();
