...
public class SowDrugCostSum extends android.support.v4.app.Fragment {
    ...
    public void setData(){
        ...
        for (int i=1; i<=DrugIdList.size();i++){
            ...
            TextView tvDay = (TextView)custom.findViewById(R.id.sum_per_day);
            ...
            tvDay.setText(Usage.get(position).toString());
            tvWeek.setText(hc.df3(hc.string2double(Total.get(position))));
            Double cost = (hc.string2double9df(Total.get(position))*hc.string2double(drugList.get(1)));//total*price
            TotalCost = TotalCost+cost;
            tvMonth.setText(hc.df2(cost));
